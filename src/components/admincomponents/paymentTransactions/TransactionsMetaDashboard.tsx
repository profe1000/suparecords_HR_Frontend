...
import { TransactionListResponse } from "../../../apiservice/transactions-service.type";
...
export default function TransactionsMetaDashboard({ meta }: Props) {
  const [showPaymentMethods, setShowPaymentMethods] = useState(false);

  if (!meta) return null;

  const metricCards = [
    { label: "Total Transactions", value: meta.total || 0, icon: "📋", color: "text-blue-600" },
    { label: "Paid Transactions", value: meta.paid_transactions || 0, icon: "✅", color: "text-emerald-600" },
    { label: "Pending Transactions", value: meta.pending_transactions || 0, icon: "⏳", color: "text-amber-600" },
    { label: "Amount Settled", value: currency(meta.amount_paid || 0), icon: "💚", color: "text-emerald-600" },
    { label: "Inflow", value: currency(meta.inflow_summary?.amount || 0), icon: "📈", color: "text-blue-600" },
    { label: "Outflow", value: currency(meta.outflow_summary?.amount || 0), icon: "📉", color: "text-rose-600" },
  ];
                    ...
                    <div key={idx} className={`rounded-lg border-2 ${colors.border} ${colors.bg} p-4`}>
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className={`mt-1 text-lg font-bold ${colors.text}`}>{currency(dist.amount)}</p>
                        </div>

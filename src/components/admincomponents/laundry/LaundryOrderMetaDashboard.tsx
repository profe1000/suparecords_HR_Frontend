...
import { LaundryOrderListResponse } from "../../../apiservice/laundry-orders-service.type";

interface LaundryOrderMetaDashboardProps {
  meta: LaundryOrderListResponse["meta"];
}

const paymentMethodColors: Record<string, { bg: string; border: string; text: string; icon: string }> = {
  "1": { bg: "bg-blue-50", border: "border-blue-200", text: "text-blue-700", icon: "💰" }, // Cash
  "2": { bg: "bg-green-50", border: "border-green-200", text: "text-green-700", icon: "💳" }, // Card
  "3": { bg: "bg-purple-50", border: "border-purple-200", text: "text-purple-700", icon: "🏦" }, // Bank
  "4": { bg: "bg-orange-50", border: "border-orange-200", text: "text-orange-700", icon: "📱" }, // Mobile
};

export default function LaundryOrderMetaDashboard({ meta }: LaundryOrderMetaDashboardProps) {
  ...
  const metricCards = [
    ...
    {
      label: "Order Items",
      value: meta.number_of_order_items || 0,
      icon: "👕",
      color: "text-purple-600",
    },
    {
      label: "Paid Orders",
      value: meta.paid_transactions || 0,
      icon: "✅",
      color: "text-emerald-600",
    },
    {
      label: "Pending Orders",
      value: meta.pending_transactions || 0,
      icon: "⏳",
      color: "text-amber-600",
    },
    {
      label: "Amount Paid",
      value: `₦${(meta.amount_paid || 0)?.toLocaleString()}`,
      icon: "💚",
      color: "text-emerald-600",
    },
    {
      label: "Amount On Credit",
      value: `₦${(meta.amount_on_credit || 0)?.toLocaleString()}`,
      icon: "📊",
      color: "text-rose-600",
    },
  ];
            ...
            <div className="border-t border-slate-200 p-4">
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
                {meta.distribution_by_payment_method.map((dist, idx) => {
                  const methodId = dist.payment_method_id?.toString() || "1";

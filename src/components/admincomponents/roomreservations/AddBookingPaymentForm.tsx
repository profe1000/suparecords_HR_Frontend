...
import { Booking, CreatePaymentPayload, PaymentStatus } from "./roomReservations.types";
...
export default function AddBookingPaymentForm({ formId, branchId, reservation, onSubmit }: Props) {
  const [paymentMethods, setPaymentMethods] = useState<PaymentMethod[]>([]);
  const [values, setValues] = useState<CreatePaymentPayload>({
  ...
  const update = <K extends keyof CreatePaymentPayload>(
    key: K,
  ...
  return (
    ...
    >
      <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-600">
        Booking: <span className="font-medium text-slate-900">{reservation.booking_reference}</span>
      </div>
